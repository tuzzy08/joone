...
- The sessions panel has since been refined again so it no longer expands or collapses the page at all. The desktop shell now renders the full saved-session list inside a fixed-height scrollable region, keeping the sidebar stable while preserving the conversation-derived session labels.
