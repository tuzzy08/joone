...
- The desktop workflow now uploads release assets through `tauri-apps/tauri-action@v0` using the canonical `assetNamePattern`, and uploads workflow artifacts explicitly through `actions/upload-artifact@v4`. A temporary explicit `gh release upload` path was removed after hosted CI proved it caused duplicate release assets.
