...
- The latest packaging slice adds `src/desktop/releaseMetadata.ts` as the desktop release naming source of truth. `.github/workflows/desktop-build.yml` now computes release metadata from `src-tauri/tauri.conf.json` before invoking `tauri-apps/tauri-action`, and passes explicit `tagName`, `releaseName`, `releaseBody`, and supported `assetNamePattern` values so desktop release naming stays aligned with the app metadata.
- The latest packaging follow-up adds `src/desktop/smokeTestBundles.ts`, which performs platform-native post-build installer sanity checks in CI: Windows MSI administrative extraction via `msiexec /a`, Linux AppImage extraction via `--appimage-extract`, and macOS DMG mount validation via `hdiutil attach` / `detach`. `.github/workflows/desktop-build.yml` now runs this smoke step immediately after bundle validation for every desktop matrix job.
- The latest hosted-CI hardening slice used real GitHub Actions runs to validate the packaging pipeline end to end. That surfaced two workflow issues that local tests had missed: unsupported `tauri-action@v0` inputs and a Windows release-asset upload gap even though the MSI built successfully.
- The desktop workflow now fixes that by publishing validated bundles explicitly with `src/desktop/publishReleaseAssets.ts` (`gh release upload --clobber`) after the smoke checks pass, and by uploading workflow artifacts explicitly through `actions/upload-artifact@v4`. This makes release uploads and workflow artifacts deterministic instead of relying on deprecated/unsupported Tauri action inputs.
- A Windows-specific follow-up fix was required after the first hosted run with the explicit publish path: several desktop CLI helper scripts were using a direct-execution guard that compared `import.meta.url` with `new URL(process.argv[1], "file:").href`, which can silently fail on Windows path shapes.
- `src/desktop/cliEntry.ts` now centralizes direct-execution detection so `releaseMetadata`, `publishReleaseAssets`, `validateBundles`, and `smokeTestBundles` all execute correctly on both Windows and POSIX runners. This is important because those scripts can otherwise exit successfully while doing nothing, which hides CI bugs.
...
2.  **Next delivery slice:** inspect the next hosted run after the Windows CLI-entry fix and confirm the GitHub release plus workflow artifacts now contain all three platform bundles with stable names, then continue with any remaining release polish or manual packaged-app smoke testing.
