...
- The sessions panel has since been refined again so it stays compact by default without losing discoverability. The desktop shell now shows only the first three saved sessions initially, exposes `View more` / `Show fewer`, and keeps the expanded list inside a fixed-height scrollable region so the sidebar stays stable while preserving the conversation-derived session labels.
